					...
					<view class="padding-horizontal-main padding-top-main">
						...
						<view v-if="(user.avatar || null) == null" class="form-gorup oh">
							...
							<button class="fl bg-white br-0 lh-0 padding-0 margin-left-xxl" hover-class="none"
								...
								<image :src="(user_avatar || default_avatar)" mode="widthFix"
						...
						<view v-if="(user.nickname || null) == null" class="form-gorup oh">
							...
							<view class="fr user-nickname-container">
								<input :type="(application_client_type == 'weixin') ? 'nickname' : 'text'"
									name="nickname" maxlength="16" placeholder-class="cr-grey" class="cr-base"
									placeholder="输入昵称1~16个字符">
							</view>
						</view>

						<view class="bottom-fixed padding-main">
							<button class="bg-main br-main cr-white round text-size-sm" type="default"
								form-type="submit" hover-class="none"
								:disabled="form_submit_disabled_status">保存</button>
						</view>
					</view>
				</form>
			</view>
		...
		methods: {
			...
			init_config(status) {
				if ((status || false) == true) {
					this.setData({
						...
						interval_time: parseInt(app.globalData.get_config(
							'config.common_app_user_base_popup_interval_time', 1800))
					});
				} else {
			...
			init(type = '') {
				...
				if (this.pages.indexOf(type) != -1 && this.client.indexOf(this.application_client_type) != -1) {
					...
					// 状态
					var status = (user == null) ? false : (((user.avatar || null) == null || (user.nickname || null) ==
						null) ? true : false);
					// 间隔时间
					var cache_time = parseInt(uni.getStorageSync(this.cache_key) || 0);
					var current_time = Date.parse(new Date()) / 1000;
					if (status && !this.popup_status && cache_time > 0 && current_time < cache_time + parseInt(this
							.interval_time)) {
						status = false;
					}
					this.setData({
