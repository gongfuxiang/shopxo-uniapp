<template>
<view class="page">
    <form @submit="form_submit" class="form-container oh">
        ...
        <view v-if="home_user_address_map_status == 1" class="form-gorup bg-white">
            ...
            <view @tap="choose_location_event" class="form-gorup-text">
                <view v-if="(user_location || null) == null && (address_data.address || null) == null" class="cr-gray">请选择地理位置</view>
                <view v-else class="cr-base">{{((user_location || null) != null && (user_location.name || null) != null) ? user_location.name+' ' : ''}}{{user_location.address || address_data.address || ''}}</view>
            </view>
        </view>
        ...
        <!-- 身份证信息 -->
        <view v-if="home_user_address_idcard_status == 1" class="idcard-container">
            <view class="form-gorup bg-white">
        ...
        <button class="submit-fixed submit-bottom" type="default" form-type="submit" hover-class="none" :disabled="form_submit_disabled_status">保存</button>
    </form>
</view>
</template>
...
export default {
  ...
  methods: {
    ...
    form_submit(e) {
      ...
      if (self.home_user_address_map_status == 1) {
        validation.push({
          fields: "lng",
          msg: "请选择地理位置"
        });
      ...
      if (self.home_user_address_idcard_status == 1) {
        // 验证
        validation.push({
          fields: "idcard_name",
          msg: "请填写身份证姓名"
        ...
        form_data['idcard_front'] = self.idcard_images_data.idcard_front || '';
        form_data['idcard_back'] = self.idcard_images_data.idcard_back || '';
      }
      ...
      form_data['is_default'] = form_data.is_default == true ? 1 : 0; // 地理位置
      ...
      if ((self.address_data || null) != null) {
        if ((lng || null) == null) {
          lng = self.address_data.lng || 0;
        }

        if ((lat || null) == null) {
          lat = self.address_data.lat || 0;
        }
      }

      form_data['lng'] = lng;
      form_data['lat'] = lat; // 验证提交表单
