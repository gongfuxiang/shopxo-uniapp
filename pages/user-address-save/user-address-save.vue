    ...
    <view class="page-bottom-fixed">
        <form @submit="form_submit" class="form-container">
            <view class="padding-main oh">
                    ...
                    <view class="form-gorup">
                        <view class="flex-row jc-sb br-b-f5 padding-vertical-sm pr">
                            ...
                            <view class="flex-1 flex-width flex-col">
                                <view v-if="home_user_address_map_status == 1" @tap="choose_location_event" class="flex-row jc-sb align-c br-b-dashed padding-bottom-sm margin-bottom-sm">
                                    ...
                                    <view class="flex-row align-c">
                                        <view class="flex-1 flex-width">
                                            <view @tap="open_ragion" class="single-text">
                                                <text v-if="this.province_name"> {{ province_name }}{{ city_name ? "-" + city_name : "" }}{{ county_name ? "-" + county_name : "" }} </text>
                                ...
                                <block v-else>
                                    <view v-if="(address_data.province_name || null) != null" class="padding-top-xxxl">
                        ...
                        <view v-if="is_user_address_discern == 1" class="pr padding-top-sm">
                            <view class="intelligent-identification oh" :style="identfication_show ? 'height:254rpx;' : 'height:0;'">
                                <view class="bg-base border-radius-main padding-main flex-col jc-sb">
                                    <textarea @input="address_discern_value_event" class="margin-0" placeholder="粘贴整段地址，自动拆分姓名、电话及地址" placeholder-class="cr-grey" />
                                    <cover-view class="self-e padding-top-sm">
                                        <button type="default" size="mini" class="cr-base text-size-xs margin-right-sm" @tap="address_discern_submit_event">清除</button>
                                        <button type="default" size="mini" class="round bg-main cr-white br-main text-size-xs" @tap="address_discern_submit_event">识别</button>
                                    </cover-view>
                                </view>
                            </view>
                            <view class="tc padding-top-main" @tap="identfication_change">
                                <text class="padding-right-sm">地址粘贴板</text>
                                <iconfont name="icon-fenlei-top" size="28rpx" color="#666" :class="identfication_show ? '' : 'rotate-180'"></iconfont>
                            </view>
                        </view>
                    </view>
                </view>
                ...
                <view class="bottom-fixed padding-vertical-main">
                ...
                success: (res) => {
                    if (res.data.code == 0) {
                        ...
                        this.setData({
                            ...
                            alias_eidt_text: ads_data.alias || "",
                        });
                        if (this.alias_list.indexOf(ads_data.alias) === -1) {
                            this.setData({
                                alias_eidt_text: ads_data.alias || "",
                            });
                        }
