<template>
    <!-- 直播选项卡列表 -->
    <view class="plugins-live-tabs" :class="'plugins-live-tabs-' + propKey" :style="style_container">
        <view :style="style_img_container">
            <component-diy-modules-tabs-view :propKey="propKey" :propValue="plugins_live_tabs" :propIsTop="top_up == '1'" :propTop="sticky_top" :propStyle="tabs_style" :propsTabsContainer="tabs_container" :propsTabsImgContainer="tabs_img_container" :propCustomNavHeight="propIsTabsUseSafeDistance ? (propCustomNavHeight * 2 + 'rpx') : '0rpx'" :propTabsBackground="tabs_background" :propTabsSlidingFixedBg="tabs_sliding_fixed_bg" @onTabsTap="tabs_click_event"></component-diy-modules-tabs-view>
            <view :style="plugins_live_margin_top">
                <view :style="plugins_live_container">
                    <view :style="plugins_live_img_container">
                        <component-diy-plugins-live-list :propKey="diy_key" :propValue="plugins_live_tabs" :propIsCommonStyle="false"></component-diy-plugins-live-list>
                    </view>
                </view>
            </view>
        </view>
...
<script>
    const app = getApp();
    import { common_styles_computer, common_img_computer, padding_computer, margin_computer, background_computer, gradient_computer, radius_computer, isEmpty, box_shadow_computer, border_computer, old_border_and_box_shadow, old_margin, old_padding, old_radius } from '@/common/js/common/common.js';
    import componentDiyModulesTabsView from '@/pages/diy/components/diy/modules/tabs-view';
    import componentDiyPluginsLiveList from '@/pages/diy/components/diy/plugins-live-list';
    ...
    export default {
        ...
        data() {
            return {
                ...
                diy_key: '',
                // 选项卡背景设置
                plugins_live_margin_top: '',
                tabs_container: '',
                tabs_img_container: '',
                tabs_sliding_fixed_bg: '',
                // 直播区域背景设置
                plugins_live_container: '',
                plugins_live_img_container: '',
                // #ifdef MP
        ...
        methods: {
            ...
            init() {
                ...
                new_data.content.theme = new_data.content.plugins_live_theme;
                new_data.content.data_type = new_tabs_data.data_type;
                new_data.content.keywords = new_tabs_data.keywords;
                new_data.content.live_status = new_tabs_data.live_status || [];
                new_data.content.is_recommended = new_tabs_data.is_recommended;
                new_data.content.carousel_col = new_data.content.plugins_live_carousel_col;
                ...
                const { tabs_bg_color_list = [], tabs_bg_direction = '', tabs_bg_background_img_style = '', tabs_bg_background_img = [], tabs_radius = old_radius, tabs_padding = old_padding, plugins_live_content_color_list = [], plugins_live_content_direction = '', plugins_live_content_background_img_style = '', plugins_live_content_background_img = [], plugins_live_content_margin = old_margin, plugins_live_content_padding = old_padding, plugins_live_content_radius = old_radius } = new_style;
                // 选项卡背景设置
                const tabs_data = {
                ...
                // 直播区域背景设置
                const plugins_live_content_data = {
                    color_list: plugins_live_content_color_list,
                    direction: plugins_live_content_direction,
                    background_img_style: plugins_live_content_background_img_style,
                    background_img: plugins_live_content_background_img,
                };
                const plugins_live_content = new_style?.plugins_live_content || old_border_and_box_shadow;
                ...
                this.setData({
                    ...
                    plugins_live_margin_top: 'margin-top:' + (new_style?.plugins_live_content_spacing || 0) * 2 + 'rpx',
                    ...
                    plugins_live_container: gradient_computer(plugins_live_content_data) + margin_computer(plugins_live_content_margin) + radius_computer(plugins_live_content_radius) + box_shadow_computer(plugins_live_content) + border_computer(plugins_live_content) + 'overflow: hidden;',
                    plugins_live_img_container: background_computer(plugins_live_content_data) + padding_computer(plugins_live_content_padding) + 'box-sizing: border-box;overflow: hidden;',
                });
            },
            // tabs切换事件
            tabs_click_event(index) {
                let new_data = JSON.parse(JSON.stringify(this.propValue));
                const tab = new_data.content.tabs_list[index];
                new_data.content.theme = new_data.content.plugins_live_theme;
                new_data.content.data_type = tab.data_type;
                new_data.content.keywords = tab.keywords;
                new_data.content.live_status = tab.live_status || [];
                new_data.content.is_recommended = tab.is_recommended;
                new_data.content.carousel_col = new_data.content.plugins_live_carousel_col;
                new_data.content.data_list = tab.data_list;
                new_data.content.data_auto_list = tab.data_auto_list;
                new_data.content.data_ids = tab.data_ids;
                new_data.content.number = tab.number;
                new_data.content.order_by_type = tab.order_by_type;
                new_data.content.order_by_rule = tab.order_by_rule;
                new_data.content.field_show = new_data.content.field_show;
                new_data.content.is_cover = tab.is_cover;
                this.setData({
                    tabs_index: index,
                    plugins_live_tabs: new_data,
                    diy_key: Math.random(),
            ...
            getTop() {
                ...
                query
                    .select('.plugins-live-tabs-' + this.propKey)
