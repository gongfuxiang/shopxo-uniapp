                            ...
                            <block v-else>
                                <block v-if="!isEmpty(item)">
                                    <view class="oh pr" :class="img_size">
                                        ...
                                        <view v-else :style="img_size">
                                            <image-empty :propImageSrc="get_item_image(item)" :propStyle="content_img_radius" propErrorStyle="width: 100rpx;height: 100rpx;"></image-empty>
                                        </view>
                                        <!-- 角标 -->
                                ...
                                <view v-if="is_show('title') || is_show('simple_desc') || is_show('price') || is_show('original_price') || is_show_groupbuy_tags || form.is_shop_show == '1'" class="flex-col flex-1 jc-sb content gap-10" :style="content_style">
                                    ...
                                    <view class="flex-row align-c jc-sb gap-8 oh">
                                        ...
                                        <view v-if="form.is_shop_show == '1'" class="flex-shrink" :data-value="item.url" @tap.stop="url_event">
    ...
    export default {
        ...
        methods: {
                ...
                if (new_form != null && new_style != null) {
                    ...
                    if (this.propIsUseAuto) {
                        ...
                        if (!isEmpty(new_form.data_list) && new_form.data_type == '0') {
                            new_list = new_form.data_list.map((item) => this.normalize_list_item(item));
                        } else if (!isEmpty(new_form.data_auto_list) && new_form.data_type == '1') {
                            new_list = new_form.data_auto_list.map((item) => this.normalize_list_item(item));
                        }
                    } else {
            ...
            normalize_list_item(item) {
                if ((item.data || null) != null) {
                    const data = item.data || {};
                    return {
                        ...data,
                        title: !isEmpty(item.new_title) ? item.new_title : data.title,
                        new_cover: item.new_cover,
                        images: data.images || data.cover,
                    };
                }
                return {
                    ...item,
                    images: item.images || item.cover,
                };
            },
            get_item_image(item) {
                return item.images || item.cover || '';
            },
                ...
                uni.request({
                    ...
                    success: (res) => {
                        ...
                        if (body.code == 0 && Array.isArray(body.data)) {
                            new_list = body.data.map((child) => this.normalize_list_item(child));
                        }
                        this.setData({
                            list: new_list,
                            shop_content_list: get_swiper_list(new_list, new_form.carousel_col, new_style.rolling_fashion),
