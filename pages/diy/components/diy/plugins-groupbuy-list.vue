        ...
        <view class="oh" :style="style_img_container">
            <view :class="outer_class" :style="outer_style">
                <block v-if="!['5'].includes(theme)">
                    <view v-for="(item, index) in list" :key="index" class="pr oh" :style="layout_style" :data-index="index" :data-value="item.url" @tap.stop="url_event">
                            ...
                            <block v-if="theme == '6'">
                                <view :class="['flex-row align-c jc-sb ptb-15 mlr-10 gap-20', { 'br-b-e': index != list.length - 1 }]">
                                    ...
                                    <view v-if="is_show('price') && !isEmpty(item.min_price)" class="num flex-row align-c nowrap flex-shrink" :style="'color:' + new_style.shop_price_color">
                                        <text class="identifying" :style="price_symbol">{{ item.show_price_symbol }}</text>
                                        <text :style="price_style">{{ item.min_price }}</text>
                                        <text v-if="is_show('price_unit')" class="identifying" :style="price_unit">{{ item.show_price_unit }}</text>
                                    </view>
                                </view>
                            </block>
                            <block v-else>
                                <block v-if="!isEmpty(item)">
                                    <view class="oh pr" :class="img_size">
                                        ...
                                        <view v-else :style="img_size">
                                            <image-empty :propImageSrc="get_item_cover(item)" :propStyle="content_img_radius" propErrorStyle="width: 100rpx;height: 100rpx;"></image-empty>
                                        </view>
                                        <!-- 角标 -->
                                ...
                                <view v-if="is_show('title') || is_show('simple_desc') || is_show('price') || is_show('original_price') || is_show_groupbuy_tags || form.is_shop_show == '1'" class="flex-col flex-1 jc-sb content gap-10" :style="content_style">
                                    ...
                                    <view class="flex-row align-c jc-sb gap-8 oh">
                                        <view :class="['flex-1 oh', form.is_price_solo == '1' ? 'flex-row align-c nowrap' : 'flex-col gap-5']">
                                            <view v-if="is_show('price') && !isEmpty(item.min_price)" class="num flex-row align-c nowrap" :style="'color:' + new_style.shop_price_color">
                                            ...
                                            <view v-if="show_content && is_show('original_price') && !isEmpty(item.min_original_price)" :class="['text-size-xss flex-row', form.is_price_solo == '1' ? 'margin-left-xs' : '']">
                                        ...
                                        <view v-if="form.is_shop_show == '1'" class="flex-shrink" :data-index="index" :data-value="item.url" @tap.stop="url_event">
                    ...
                    <swiper circular="true" :autoplay="new_style.is_roll == '1'" :interval="new_style.interval_time * 1000" :duration="500" :next-margin="new_style.rolling_fashion == 'translation' ? '-' + content_outer_spacing_magin : '0rpx'" :display-multiple-items="slides_per_group" :style="{ width: '100%', height: new_style.content_outer_height * new_scale + 'px' }">
                        <swiper-item v-for="(item1, index1) in shop_content_list" :key="index1">
                            <view class="flex-row wh-auto ht-auto" :style="outer_style">
                                <view v-for="(item, index) in item1.split_list" :key="index" class="pr oh" :style="layout_style" :data-index="index1" :data-split-index="index" :data-value="item.url" @tap.stop="url_event">
                                    <template v-if="!isEmpty(item)">
                                        <view :class="layout_type" :style="layout_img_style">
                                            ...
                                            <view v-if="is_show('title') || is_show('simple_desc') || is_show('price') || is_show_groupbuy_tags || is_show('original_price') || form.is_shop_show == '1'" class="flex-col flex-1 jc-sb content gap-10" :style="content_style">
                                                ...
                                                <view class="flex-row align-c jc-sb gap-8 oh">
                                                    <view class="flex-row align-c nowrap flex-1 oh">
                                                        <view v-if="is_show('price') && !isEmpty(item.min_price)" class="num flex-row align-c nowrap" :style="'color:' + new_style.shop_price_color">
                                                            <text :style="price_symbol">{{ item.show_price_symbol }}</text>
                                                            <text :style="price_style">{{ item.min_price }}</text>
                                                            <text v-if="is_show('price_unit')" :style="price_unit">{{ item.show_price_unit }}</text>
                                                        </view>
                                                        <view v-if="show_content && is_show('original_price') && !isEmpty(item.min_original_price)" class="text-size-xss flex-row margin-left-xs">
                                                            <text :class="['original-price text-line-1', { 'flex-1': form.is_price_solo == '1' }]" :style="original_price">
                                                                {{ item.show_original_price_symbol }}{{ item.min_original_price }}
                                                                <block v-if="is_show('original_price_unit')">
                                                                    {{ item.show_original_price_unit }}
                                                    ...
                                                    <view v-if="form.is_shop_show == '1'" class="flex-shrink" :data-index="index1" :data-split-index="index" :data-value="item.url" @tap.stop="url_event">
                                                        <block v-if="form.shop_type == 'text'">
                                                            <view class="plr-11 padding-vertical-xs round" :style="button_style + ('color:' + new_style.shop_button_text_color)">{{ form.shop_button_text }}</view>
                                                        </block>
                                                        <view v-else class="round padding-horizontal-sm ptb-5" :style="button_gradient">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            init() {
                ...
                if (new_form != null && new_style != null) {
                    ...
                    if (this.propIsUseAuto) {
                        new_list = this.resolve_list_data(new_form);
                    } else {
            ...
            resolve_list_data(new_form) {
                // 指定数据
                if (new_form.data_type == '0') {
                    if (!isEmpty(new_form.data_list)) {
                        return new_form.data_list.map((item) => this.normalize_appoint_item(item));
                    }
                    return [];
                }
                // 自动数据（与 shopxo-diy 一致，直接使用 data_auto_list）
                if (!isEmpty(new_form.data_auto_list)) {
                    return new_form.data_auto_list;
                }
                return [];
            },
            normalize_appoint_item(item) {
                const data = item.data;
                if (data != null && !Array.isArray(data) && typeof data === 'object' && !isEmpty(data)) {
                    return {
                        ...data,
                        title: !isEmpty(item.new_title) ? item.new_title : data.title,
