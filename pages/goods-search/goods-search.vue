...
<component-popup :prop-show="is_show_popup_form" prop-position="left" @onclose="popup_form_event_close">
  <form @submit="form_submit_event" class="popup-form oh bg-white">
    <view class="search-map">
      ...
      <!-- 品牌 -->
      <view v-if="((brand_list || null) != null && brand_list.length > 0) || ((search_map_info.brand || null) != null)" class="map-item">
        <view class="map-nav tc">
          <text>品牌</text>
          <text class="arrow-bottom" v-if="brand_list.length > 3" @tap="more_event" data-value="brand_list">更多</text>
        </view>
        <view v-if="(search_map_info.brand || null) != null" class="map-content brand-info oh bg-white">
          ...
          <view class="info-right fr">
            ...
            <view v-if="(search_map_info.brand.describe || null) != null" class="info-desc multi-text">{{search_map_info.brand.describe}}</view>
          </view>
        </view>
        <view v-else class="map-content map-images-text-items map-brand-container oh bg-white" :style="'height:' + map_fields_list.brand_list.height + ';'">
          <block v-for="(item, index) in brand_list" :key="index">
            <view :class="'fl tc single-text ' + (item.active == 1 ? 'active' : '')" @tap="map_item_event" :data-index="index" data-field="brand_list">
      ...
      <!-- 价格 -->
      <view v-if="(screening_price_list || null) != null && screening_price_list.length > 0" class="map-item">
        <view class="map-nav tc">
          <text>价格</text>
          <text class="arrow-bottom" v-if="screening_price_list.length > 3" @tap="more_event" data-value="screening_price_list">更多</text>
        </view>
        <view class="map-content map-text-items screening-price-container oh bg-white" :style="'height:' + map_fields_list.screening_price_list.height + ';'">
          <block v-for="(item, index) in screening_price_list" :key="index">
            <view :class="'fl ' + (item.active == 1 ? 'active' : '')" @tap="map_item_event" :data-index="index" data-field="screening_price_list">{{item.name}}</view>
          </block>
        </view>
      </view>

      <!-- 属性 -->
      <view v-if="(goods_params_list || null) != null && goods_params_list.length > 0" class="map-item">
        <view class="map-nav tc">
          <text>属性</text>
          <text class="arrow-bottom" v-if="goods_params_list.length > 3" @tap="more_event" data-value="goods_params_list">更多</text>
        </view>
        <view class="map-content map-text-items goods-params-container oh bg-white" :style="'height:' + map_fields_list.goods_params_list.height + ';'">
          <block v-for="(item, index) in goods_params_list" :key="index">
            <view :class="'fl ' + (item.active == 1 ? 'active' : '')" @tap="map_item_event" :data-index="index" data-field="goods_params_list">{{item.value}}</view>
          </block>
        </view>
      </view>

      <!-- 规格 -->
      <view v-if="(goods_spec_list || null) != null && goods_spec_list.length > 0" class="map-item">
        <view class="map-nav tc">
          <text>规格</text>
          <text class="arrow-bottom" v-if="goods_spec_list.length > 3" @tap="more_event" data-value="goods_spec_list">更多</text>
        </view>
        <view class="map-content map-text-items goods-spec-container oh bg-white" :style="'height:' + map_fields_list.goods_spec_list.height + ';'">
          <block v-for="(item, index) in goods_spec_list" :key="index">
            <view :class="'fl ' + (item.active == 1 ? 'active' : '')" @tap="map_item_event" :data-index="index" data-field="goods_spec_list">{{item.value}}</view>
          </block>
        </view>
      </view>
...
export default {
  ...
  onLoad(params) {
    // 启动参数处理
    params = app.globalData.launch_params_handle(params); // 初始参数
  ...
  methods: {
    ...
    request_map_handle() {
      ...
      post_data['page'] = this.data_page; // 指定分类、品牌

      post_data['category_id'] = params['category_id'] || 0;
      post_data['brand_id'] = params['brand_id'] || 0; // 搜索条件
    ...
    map_remove_event(e) {
      ...
      data['post_data']['wd'] = ''; // 品牌、分类、价格、属性、规格
