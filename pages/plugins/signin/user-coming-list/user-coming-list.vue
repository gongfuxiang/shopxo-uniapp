  ...
  <view class="data-list">
    <view v-for="(item, index) in data_list" :key="index" class="item bg-white spacing-mb" v-if="data_list.length > 0">
      <view class="base oh br-b">
        <image :src="item.user.avatar" class="avatar dis-block fl" mode="widthFix" @tap="avatar_event" :data-value="item.user.avatar"></image>
        <text class="fr nickname cr-gray">{{item.user.user_name_view || ''}}</text>
      </view>
      <view class="content">
        <view class="single-text">
          <text class="title cr-base">奖励积分</text>
          <text class="value">{{item.integral}}</text>
        </view>
        <view class="single-text">
          <text class="title cr-base">签到时间</text>
