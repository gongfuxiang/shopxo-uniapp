            ...
            <view class="padding-horizontal-main padding-top-main">
                ...
                <view class="border-radius-main padding-main spacing-mb team-status-panel pr" :class="team.status == 1 || team.status == 2 ? 'team-status-panel-success' : 'bg-white'">
                    <block v-if="team.status == 0">
                        <view class="tc fw-b text-size-md">{{ $t('bargain.bargaining_need') }}<text class="cr-red">{{ team.remain_number }}</text>{{ $t('bargain.people_success') }}</view>
                        ...
                        <view class="team-bargain-price-block margin-top-main">
                            <view class="team-bargain-current-row">
                                <text class="team-bargain-label">{{ $t('bargain.current_price') }}</text>
                            ...
                            <view class="team-bargain-floor-row">{{ $t('bargain.floor_price') }} {{ currency_symbol }}{{ team.floor_price_text || team.floor_price }}</view>
                            <view class="team-bargain-progress-wrap">
                                ...
                                <view class="team-bargain-progress-meta text-size-xs cr-grey">
                                    <text>{{ $t('bargain.already_cut') }} {{ currency_symbol }}{{ team.cut_total_price_text || team.cut_total_price }}</text>
                                    <text>{{ $t('bargain.cut_remain') }} {{ currency_symbol }}{{ team.remain_price_text || team.remain_price }}</text>
                                </view>
                            </view>
                        </view>
                    </block>
                    <block v-else-if="team.status == 1 || team.status == 2">
                        ...
                        <view class="team-status-success tc">
                            ...
                            <view class="team-success-title">{{ $t('bargain.bargain_success') }}</view>
                            <view class="team-success-desc">
                                {{ $t('bargain.success_go_pay') }}
                                <text class="team-success-help-tag">{{ bargain.help_number || team.help_number || 0 }}{{ $t('bargain.people_help_cut_suffix') }}</text>
                            </view>
                            <view class="team-bargain-price-block team-success-price-block">
                                <view class="team-bargain-current-row team-success-current-row">
                                    ...
                                    <text class="team-bargain-cut-done-badge">{{ $t('bargain.already_cut') }} {{ currency_symbol }}{{ team.cut_total_price_text || team.cut_total_price }}</text>
                                </view>
                                ...
                                <view v-if="(team.floor_price_text || team.floor_price || null) != null" class="team-bargain-floor-row team-success-floor-row">{{ $t('bargain.floor_price') }} {{ currency_symbol }}{{ team.floor_price_text || team.floor_price }}</view>
                            </view>
                        </view>
                    </block>
                    <block v-else-if="team.status == 3 || team.status == 4">
                        <view class="team-status-fail tc">
                            ...
                            <view class="fw-b text-size-lg cr-red margin-top-main">{{ $t('bargain.bargain_fail') }}</view>
                            <view class="cr-grey text-size-xs margin-top-sm">{{ $t('bargain.bargain_fail_tips') }}</view>
                        </view>
                    </block>

                    <view v-if="team.status == 0" class="team-action-row margin-top-main flex-row align-c jc-c flex-wrap gap-10">
                        <block v-if="user == null">
                            <button class="btn round bg-main cr-white text-size-md" type="default" @tap="login_event" hover-class="none">{{ $t('bargain.login_to_help') }}</button>
                        </block>
                        <block v-else>
                            <button v-if="help_action.can_help == 1" class="btn round bg-main cr-white text-size-md" type="default" @tap="help_cut_event" hover-class="none">{{ $t('bargain.help_now') }}</button>
                            <button v-else-if="help_action.has_helped == 1" class="btn round bg-main cr-white text-size-md" type="default" disabled hover-class="none">{{ $t('bargain.already_helped') }}</button>
                            <text v-else-if="help_action.is_owner == 1" class="cr-grey text-size-sm">{{ $t('bargain.invite_help') }}</text>
                            <text v-else class="cr-grey text-size-sm">{{ help_action.tips || this.$t('bargain.cannot_help') }}</text>
                        ...
                        <button class="btn round bg-white br-main cr-main text-size-md" type="default" @tap="popup_share_event" hover-class="none">{{ $t('bargain.invite_help_cut') }}</button>
                    </view>
                    ...
                    <view v-if="help_action.is_owner == 1 && (team.status == 0 || team.status == 1) && team_pay.has_system_order != 1 && team.has_system_order != 1" class="tc margin-top-main">
                        <text class="cr-grey text-size-xs" @tap="team_cancel_event">{{ $t('bargain.cancel_launch') }}</text>
                    </view>
                </view>

                <view v-if="help_list.length > 0" class="border-radius-main bg-white padding-main spacing-mb">
                    <component-bargain-dot-title :propTitle="$t('bargain.bargain_record')" propTextClass="text-size-sm"></component-bargain-dot-title>
        ...
        methods: {
                    ...
                    success: (res) => {
                        ...
                        if (res.data.code == 0) {
                            ...
                            this.setData({
                                ...
                                share_info: {
                                    title: bargain.title || goods.title || this.$t('bargain.invite_you_help'),
                                    desc: this.$t('common.still_need') + (team.remain_number || 0) + this.$t('bargain.invite_help_one'),
            ...
            help_cut_event() {
                uni.request({
                    ...
                    success: (res) => {
                        if (res.data.code == 0) {
                            app.globalData.showToast(res.data.msg || this.$t('bargain.help_success'), 'success');
                            this.get_data();
                        } else {
