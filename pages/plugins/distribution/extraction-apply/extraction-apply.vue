...
<form @submit="form_submit" class="form-container oh">
  <view class="form-gorup bg-white form-container-upload oh">
    ...
    <image class="upload-icon" src="/static/images/default-upload-icon.png" mode="aspectFill" @tap="file_upload_event"></image>
  </view>
  ...
  <view class="form-gorup bg-white">
    ...
    <view @tap="choose_location_event" class="form-gorup-text">
      ...
      <view v-else class="cr-base">{{((user_location || null) != null && (user_location.name || null) != null) ? user_location.name+' ' : ''}}{{user_location.address || ''}}</view>
    </view>
  </view>

  <view class="form-gorup">
    <view v-if="(extraction_data || null) != null && (extraction_data.status || 0) == 1" class="tips spacing-mb">
      注意：编辑信息将重新审核后方可生效
    </view>
    <button class="submit-bottom" type="default" form-type="submit" hover-class="none" :disabled="form_submit_disabled_status">提交</button>
  </view>
</form>
</view>
</template>
...
export default {
  ...
  methods: {
    ...
    form_submit(e) {
      ...
      form_data['logo'] = (this.extraction_data || null) != null ? this.extraction_data.logo || '' : ''; // 地区

      form_data["province"] = self.province_id;
      form_data["city"] = self.city_id;
      form_data["county"] = self.county_id; // 地理位置
