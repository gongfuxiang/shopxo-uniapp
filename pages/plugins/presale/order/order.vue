                        ...
                        <view class="oh margin-top flex-row gap-10" :data-value="items.goods_url" @tap="url_event">
                            ...
                            <view class="goods-base">
                                ...
                                <view class="margin-top-xs">
                                    <text class="cr-grey">{{currency_symbol}}{{items.price}} x{{items.buy_number}}</text>
                                    <text v-if="(items.spec_text || null) != null" class="cr-grey fr">{{items.spec_text}}</text>
                                </view>
                                <view class="margin-top-xs">
                                    <text>{{presale_config.goods_detail_icon}}</text>
                                    <text class="cr-price fw-b">{{currency_symbol}}{{items.total_deposit_price}}</text>
                                    <text v-if="(items.total_deduct_price || 0) > 0" class="cr-green margin-left">{{presale_config.goods_detail_deduct_text}}{{currency_symbol}}{{items.total_deduct_price}}</text>
                                </view>
                            </view>
                        </view>
                    </block>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            get_data_base() {
                uni.request({
                    url: app.globalData.get_request_url('init', 'order', 'presale'),
                    method: 'POST',
                    data: {},
                    dataType: 'json',
                    success: (res) => {
            ...
            get_data_list(is_mandatory) {
                ...
                // 获取数据
                uni.request({
                    url: app.globalData.get_request_url('index', 'order', 'presale'),
                    method: 'POST',
                    data: data,
                    dataType: 'json',
                    success: (res) => {
            ...
            pay_event(e) {
                ...
                uni.request({
                    url: app.globalData.get_request_url('buy', 'order', 'presale'),
                    method: 'POST',
                    data: {
                    ...
                    success: (res) => {
                        uni.hideLoading();
                        if (res.data.code == 0) {
                            app.globalData.url_open('/pages/buy/buy?presale_order_id='+res.data.data);
                ...
                uni.showModal({
                    ...
                    success: (result) => {
                        if (result.confirm) {
                            ...
                            uni.request({
                                url: app.globalData.get_request_url('cancel', 'order', 'presale'),
                                method: 'POST',
                                data: {
                                    ids: value,
                                },
                ...
                uni.showModal({
                    ...
                    success: (result) => {
                        if (result.confirm) {
                            ...
                            uni.request({
                                url: app.globalData.get_request_url('delete', 'order', 'presale'),
                                method: 'POST',
                                data: {
                                    ids: value,
                                },
