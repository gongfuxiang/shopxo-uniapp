            ...
            <view class="padding-horizontal-main padding-top-main">
                    ...
                    <view class="flex-row gap-20" :data-value="'/pages/plugins/groupbuy/detail/detail?id=' + groupbuy.id" @tap="url_event">
                        ...
                        <view class="flex-1 flex-width">
                            ...
                            <view class="margin-top-xs">
                                <text class="groupbuy-tag cr-white bg-main text-size-xs radius padding-horizontal-sm">{{ groupbuy.group_number }}{{ $t('common.people_team') }}</text>
                            </view>
                ...
                <view class="border-radius-main padding-main spacing-mb team-status-panel" :class="team.status == 1 ? 'team-status-panel-success' : 'bg-white'">
                    <block v-if="team.status == 0">
                        <view class="tc fw-b text-size-md">{{ $t('groupbuy.grouping_need') }}<text class="cr-red">{{ team.remain_number }}</text>{{ $t('common.person_unit') }}</view>
                    ...
                    <block v-else-if="team.status == 1">
                        <view class="team-status-success tc">
                            ...
                            <view class="team-success-title">{{ $t('groupbuy.group_success') }}</view>
                            <view class="team-success-desc cr-grey text-size-sm">{{ $t('groupbuy.group_full_success') }}</view>
                            <view class="team-success-tags flex-row jc-c flex-wrap">
                                <text class="team-success-tag">{{ $t('groupbuy.full_group') }}</text>
                                <text class="team-success-tag">{{ groupbuy.group_number }}{{ $t('common.people_team') }}</text>
                            </view>
                        </view>
                    </block>
                    <block v-else>
                        <view class="team-status-fail tc">
                            ...
                            <view class="fw-b text-size-lg cr-red">{{ $t('groupbuy.group_fail') }}</view>
                            <view class="cr-grey text-size-xs margin-top-sm">{{ $t('groupbuy.group_fail_tips') }}</view>
                        </view>
                    </block>
                    ...
                    <view class="team-members flex-row flex-wrap jc-c margin-top-main">
                        <view v-for="(item, index) in members" :key="item.user_id" class="member-item tc">
                            ...
                            <view v-if="item.is_team_leader == 1" class="leader-badge bg-main cr-white text-size-xss radius padding-horizontal-xs">{{ $t('groupbuy.team_leader') }}</view>
                    ...
                    <view v-if="team.status == 0" class="margin-top-main">
                        <button v-if="user == null" class="btn round bg-main cr-white text-size-md wh-auto" type="default" @tap="login_event" hover-class="none">{{ $t('groupbuy.login_to_join') }}</button>
                        <block v-else>
                            <button class="btn round bg-main cr-white text-size-md wh-auto margin-bottom-main" type="default" @tap="join_team_event" hover-class="none">{{ $t('groupbuy.join_now') }}</button>
                            <button class="btn round bg-white br-main cr-main text-size-md wh-auto" type="default" hover-class="none" @tap="popup_share_event">{{ $t('groupbuy.invite_friends') }}</button>
                            <view v-if="is_leader" class="tc margin-top-main">
                                <text class="cr-grey text-size-xs" @tap="team_cancel_event">{{ $t('groupbuy.cancel_open') }}</text>
                            </view>
                        </block>
                    </view>
                </view>
        ...
        methods: {
                ...
                uni.request({
                    ...
                    success: (res) => {
                        ...
                        if (res.data.code == 0) {
                            ...
                            var share_title = groupbuy.title || goods.title || this.$t('groupbuy.invite_you');
                            var share_desc = this.$t('common.still_need') + (team.remain_number || 0) + this.$t('groupbuy.people_group_invite');
            ...
            team_cancel_event() {
                uni.showModal({
                    ...
                    content: this.$t('groupbuy.confirm_cancel'),
                    success: (result) => {
                        if (result.confirm) {
