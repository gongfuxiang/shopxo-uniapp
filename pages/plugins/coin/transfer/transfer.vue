            ...
            <view class="padding-main">
                    ...
                    <view class="padding-vertical-sm flex-row align-c">
                        ...
                        <view class="padding-left-lg flex-row jc-sb align-c flex-1 flex-width">
                            <view class="flex-1 padding-right-main">
                                <input type="text" name="receive_accounts_key" :value="receive_accounts_key" placeholder-class="text-size-md cr-grey-9" :placeholder="$t('common.enter_receiving_account_number')" @input="receive_accounts_key_change" />
                            </view>
                ...
                <view class="bg-white padding-main radius-md margin-bottom-main">
                    <view class="padding-vertical-sm">
                        <text class="text-size fw-b">{{$t('transfer.transfer_currency')}}</text>
                        <view class="margin-top-xxxxl">
                            <input type="digit" name="coin" :value="coin" class="text-size-xl tc" placeholder-class="cr-grey-9" :placeholder="$t('transfer.enter_transfer_currency')" @input="coin_change" />
                            <view class="cr-grey-9 tc margin-top-main">{{$t('common.my_balance')}}<text class="cr-black fw-b">{{ accounts.platform_symbol }} {{ accounts.default_coin }}</text>
                            </view>
                        </view>
                        <view class="note padding-main radius margin-top-xxxl">
                            <input type="text" name="note" :value="note" placeholder-class="text-size-md cr-grey-9" :placeholder="$t('common.enter_transfer_remarks_info')" @input="note_change" />
                        </view>
                    </view>
                </view>
                ...
                <view class="padding-xxxl margin-top-sm">
                    <button type="default" class="transfer-btn cr-white round" @tap="transfer_event">{{$t('transfer.transfer_immediately')}}</button>
                </view>
            </view>
        </view>
        <block v-else>
    ...
    export default {
        ...
        methods: {
            ...
            transfer_event(e) {
                ...
                // 数据校验
                var validation = [
                    { fields: 'receive_accounts_key', msg: this.$t('common.enter_receiving_account_number') },
                    { fields: 'coin', msg: this.$t('transfer.enter_transfer_currency') },
                ];
                // 验证提交表单
