	...
	<view>
		...
		<view class="nav-base bg-white">
			<block v-for="(item, index) in nav_tabs_list" :key="index">
				<view :class="'item fl tc cr-gray ' + (item.value == nav_tabs_value ? 'cr-main nav-active-line' : '')" :data-index="index" :data-value="item.value" @tap="nav_tabs_event">{{item.name}}
				</view>
			</block>
		</view>
			...
			<view v-if="data_list != null" class="plugins-coupon-container padding-top-main">
				...
				<block v-if="(data_list.not_use || null) != null && data_list.not_use.length > 0 && nav_tabs_value == 'not_use'">
					<block v-for="(item, index) in data_list.not_use" :key="index">
						...
						<component-poupon-card :prop-data="item.coupon" prop-bg="#f5f5f5"></component-poupon-card>
						<!-- <view class="item spacing-mb bg-white border-radius-main">
							<view class="v-left fl">
								...
								<view v-if="(item.coupon.use_limit_type_name || null) != null" class="base-tips cr-base single-text text-size-xs">{{item.coupon.use_limit_type_name}}</view>
								<view class="base-time cr-gray single-text text-size-xs">{{item.time_start}} 至 {{item.time_end}}</view>
							</view>
					...
					<block v-for="(item, index) in data_list.already_use" :key="index">
						...
						<!-- <view class="item spacing-mb bg-white border-radius-main item-disabled">
							<view class="v-left fl">
								...
								<view v-if="(item.coupon.use_limit_type_name || null) != null" class="base-tips cr-base single-text text-size-xs">{{item.coupon.use_limit_type_name}}</view>
								<view class="base-time cr-gray single-text text-size-xs">使用 {{item.use_time_time}}</view>
							</view>
					...
					<block v-for="(item, index) in data_list.already_expire" :key="index">
						...
						<!-- <view class="item spacing-mt bg-white border-radius-main item-disabled">
							<view class="v-left fl">
								...
								<view v-if="(item.coupon.use_limit_type_name || null) != null" class="base-tips cr-base single-text text-size-xs">{{item.coupon.use_limit_type_name}}</view>
								<view class="base-time cr-gray single-text text-size-xs">过期 {{item.time_end}}</view>
							</view>
							<view class="v-right fr">
								<text class="circle"></text>
								<text>已过期</text>
