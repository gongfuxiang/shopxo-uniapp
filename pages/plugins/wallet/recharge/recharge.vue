<template>
    <view :class="theme_view">
        <component-nav-back :propName="$t('common.recharge')"></component-nav-back>
            ...
            <view class="padding-top-xxxl">
                ...
                <view class="pr padding-top-main">
                    <view class="wallet-price cr-white">
                        <view class="text-size-lg fw-b">{{$t('recharge.account_balance')}}</view>
                    ...
                    <view class="recharge-content bg-white spacing-mt">
                        <block v-if="preset_data.length > 0">
                            <view class="padding-horizontal-sm spacing-mb">{{$t('recharge.select_recharge_amount')}}</view>
                        ...
                        <view class="form-container padding-top-sm padding-horizontal-sm">
                            <view class="form-gorup border-radius-main custom-price flex-row align-c">
                                <view class="margin-right-xxl">{{preset_data.length > 0 ? $t('recharge.other') : $t('common.recharge')}}{{$t('recharge.money')}}</view>
                                <input type="digit" name="money" v-model="recharge_money_value" placeholder-class="cr-grey-9" class="cr-base text-size-md flex-1 flex-width" :placeholder="$t('recharge.enter_recharge_amount')" @input="recharge_money_value_input_event" maxlength="6" />
                            </view>
                            <view class="form-gorup form-gorup-submit spacing-mt">
                                <button class="round cr-white bg-main br-main text-size" type="default" hover-class="none" :disabled="form_submit_disabled_status" @tap="form_submit_event">{{$t('common.recharge_now')}}</button>
                            </view>
                        </view>
                        <view v-if="((recharge_desc || null) != null && recharge_desc.length > 0)" class="recharge-desc padding-top-xxl padding-horizontal-sm">
                            <view class="recharge-desc-title padding-bottom-sm">{{$t('recharge.recharge_instructions')}}</view>
        ...
        methods: {
            ...
            form_submit_event(e) {
                ...
                if ((this.recharge_money_value || null) == null) {
                    app.globalData.showToast(this.$t('recharge.enter_recharge_amount'));
                    return false;
                }
