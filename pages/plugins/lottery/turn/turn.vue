    ...
    export default {
        ...
        computed: {
            ...
            lotteryTurnBarText() {
                ...
                return this.$t('lottery.lottery.balance_label') + (t.user_wallet_money || '0.00') + this.$t('lottery.lottery.integral_label') + (t.user_integral || '0');
            },
            /** 弹窗类按钮「确认」文案（多语言） */
            confirmBtnText() {
                    ...
                    success: (res) => {
                        if (res.data.code == 0) {
                            ...
                            if (turn.enable === false) {
                                const tip = String(turn.error_tips || '').trim() || this.$t('lottery.lottery.activity_unavailable');
                            ...
                            this.resultFailTitle = turn.result_fail_title || this.$t('lottery.lottery.thanks_join');
                            this.resultFailDesc = turn.result_fail_desc || this.$t('lottery.lottery.try_again_tips');
                            this.resultSuccessTitle = turn.result_success_title || this.$t('lottery.lottery.congrats_get');
