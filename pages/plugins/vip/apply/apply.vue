<template>
    <view :class="theme_view">
        <view v-if="data_list_loding_status == 3" class="page-bottom-fixed">
            <!-- 已有会员权限 -->
            <view v-if="is_has_vip_level == 1" class="padding-main">
                <view class="padding-vertical-main border-radius-main bg-green-light cr-green">
                    <view>{{$t('apply.vip_application_been_approved')}}<template v-if="(apply_data || null) != null && (apply_data.level_name || null) != null">{{$t('apply.level')}}{{ apply_data.level_name }}</template></view>
                </view>
            ...
            <view v-else-if="(apply_data || null) != null && apply_data.status == 0" class="padding-main">
                <view class="padding-vertical-main border-radius-main bg-yellow-light cr-yellow">
                    <view>{{$t('apply.vip_application_under_review_wait_patie')}}</view>
                </view>
                ...
                <view class="bottom-fixed" :style="bottom_fixed_style">
                    <view class="bottom-line-exclude">
                        <button class="item bg-main br-main cr-white round text-size" type="default" hover-class="none" data-value="/pages/plugins/vip/apply-form/apply-form" @tap="url_event">{{$t('common.edit')}}</button>
                    </view>
                </view>
            </view>
            ...
            <view v-else-if="(apply_data || null) != null && apply_data.status == 2" class="padding-main">
                ...
                <view class="bottom-fixed" :style="bottom_fixed_style">
                    <view class="bottom-line-exclude">
                        <button class="item bg-main br-main cr-white round text-size" type="default" hover-class="none" data-value="/pages/plugins/vip/apply-form/apply-form" @tap="url_event">{{$t('apply.reapply')}}</button>
                    </view>
                </view>
            </view>
            ...
            <view v-else class="padding-main">
                ...
                <view class="bottom-fixed" :style="bottom_fixed_style">
                    <view class="bottom-line-exclude">
                        <button class="item bg-main br-main cr-white round text-size" type="default" hover-class="none" data-value="/pages/plugins/vip/apply-form/apply-form" @tap="url_event">{{$t('apply.go_fill')}}</button>
                    </view>
                </view>
            </view>
        </view>
                ...
                uni.request({
                    ...
                    success: (res) => {
                        ...
                        if (res.data.code == 0) {
                            ...
                            var is_has_vip_level = data.is_has_vip_level || 0;
                            this.setData({
                                data_list_loding_status: 3,
                                data_list_loding_msg: '',
                                is_has_vip_level: is_has_vip_level,
                                apply_data: apply_data,
                            });
                            uni.setNavigationBarTitle({
                                title: this.$t('pages.plugins-vip-apply'),
                            });
                            // 无申请记录：确认是否进入填写页（取消则留在本页，避免返回死循环）
                            if (is_has_vip_level != 1 && (apply_data || null) == null && !this.is_apply_form_confirm_done) {
                                this.setData({
                                ...
                                uni.showModal({
                                    ...
                                    success: (result) => {
                                        if (result.confirm) {
                                            app.globalData.url_open('/pages/plugins/vip/apply-form/apply-form');
                                        }
                                    },
                                });
                            }
